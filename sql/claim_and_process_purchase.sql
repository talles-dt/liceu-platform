...
BEGIN
 ...
 -- Search by any of these conditions:
 --   a) Same ID, not claimed
 --   b) Email + Stripe session ID + not claimed
 --   c) User ID + Stripe session ID + not claimed
 ...
 OR (
 user_id = p_user_id
 AND stripe_session_id = (SELECT stripe_session_id FROM pending_purchases WHERE id = p_purchase_id)
 )
 OR (
 email = p_user_email
 AND stripe_session_id = (SELECT stripe_session_id FROM pending_purchases WHERE id = p_purchase_id)
 ...
 OR (
 user_id = p_user_id
 AND stripe_session_id = purchase_record.stripe_session_id
 AND claimed = FALSE
 )
 OR (
 email = p_user_email
...
REVOKE ALL ON FUNCTION claim_and_process_purchase(uuid, text, text) FROM PUBLIC;
